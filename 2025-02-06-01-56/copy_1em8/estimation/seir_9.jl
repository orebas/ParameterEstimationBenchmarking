...
ic = [0.717, 0.689, 0.213, 0.793]
p_true = [0.723, 0.736, 0.376]
...
data_sample = Dict(vcat("t", map(x -> x.rhs, measured_quantities)) .=> CSV.read("/home/orebas/julia/no-matlab-no-worry/2025-02-06-01-56/copy_1em8/data/seir_9.csv", Tuple, header=false))
...
CSV.write("/home/orebas/julia/no-matlab-no-worry/2025-02-06-01-56/copy_1em8/estimation_results/seir_9.csv", table, header=string.(collect(keys(table))))
