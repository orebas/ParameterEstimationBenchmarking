...
### Date: 2026-05-29 18:25:20
### Generated by: src/generate_scripts.py benchmark_quoll_broad_2026-05-29 -s odepe_v2_polish -r odepe_v2_polish_run
### Environment: /usr
...
# === CAPTURE HOOK: serialize each re-solve candidate (6 fixed params) before processing ===
import Serialization
const _DUMP_N = Ref(0)
function _dump_cand(pd)
    _DUMP_N[] += 1
    try
        Serialization.serialize("/tmp/last_cand.jls", pd)
        open("/tmp/cand_counter.txt", "w") do io; print(io, _DUMP_N[]); end
    catch e; @warn "[CAP] dump fail" exception=e end
end
@eval ODEParameterEstimation begin
    function apply_prefixed_params_to_model(ode, measured_quantities, pre_fixed_params::OrderedDict)
        if length(pre_fixed_params) == 6
            try; Main._dump_cand(pre_fixed_params); catch; end
        end
        ...
        new_eqs = [Symbolics.substitute(eq.lhs, subst_dict) ~ Symbolics.substitute(eq.rhs, subst_dict) for eq in original_eqs]
        new_measured = [Symbolics.substitute(mq.lhs, subst_dict) ~ Symbolics.substitute(mq.rhs, subst_dict) for mq in measured_quantities]
        new_name = Symbol(string(ModelingToolkit.nameof(model)) * "_prefixed")
        new_system = ModelingToolkit.ODESystem(new_eqs, t, original_states, remaining_params; name = new_name)
        new_ode = isa(ode, OrderedODESystem) ? OrderedODESystem(new_system, ode.original_states, remaining_params) : new_system
...
#     per-variable log space). The true Mustache toggle controls whether
