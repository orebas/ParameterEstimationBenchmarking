...
Wallaby variant differs from numbat's builder in:
  * Three parallel metric families per cell:
    ...
    - `mbounded_*` — argmin over the M candidate branches ODEPE returned, where
                     M = the number of rows in result.csv (ODEPE's runtime-detected
                     algebraic multiplicity — it already truncates to exactly M).
                     This is oracle ranking over the returned rows: no config lookup,
                     no canonical-M overlay. Because result.csv is M-truncated,
                     `mbounded_* == oracle_*` by construction. For M=1 systems it
                     equals top-1; latent_subpopulation_branch returns 6 rows (S3
                     symmetry), so it is scored as best-of-6. This is the metric that
                     gives the algorithm credit for finding any algebraic branch.
...
# NOTE: M (algebraic multiplicity) is NOT read from config/systems.json. ODEPE
# auto-detects M at runtime (SI Groebner quotient-basis dimension) and truncates
# result.csv to exactly that many candidate branches, so M == the number of rows
# returned per cell. We use that directly (computed per cell in main()), which
# makes the M-bounded metric exactly "oracle ranking over the returned rows" — no
# config lookup, no canonical-M overlay. (daisy_mamil4 -> 2 rows, seir/slow_fast/
# biohydrogenation -> 2, latent_subpopulation_branch -> 6, M=1 systems -> 1 row.)
...
def load_metadata_notes(p: Path):
    """Return (was_terminal_fallback, polish_maxtime_exceeded, primary_method, rescue_path, source_type)."""
    if not p.exists():
    ...
    try:
        ...
        return False, False, None, None, None
    provenance = md.get("provenance") or md.get("best", {}) or {}
    notes = provenance.get("notes", []) or []
    notes_set = {str(n).lower() for n in notes}
    rescue_path = provenance.get("rescue_path")
    return (
        bool(provenance.get("was_terminal_fallback", False)) or str(rescue_path) == "direct_opt_fallback",
        "polish_maxtime_exceeded" in notes_set,
        provenance.get("primary_method"),
        rescue_path,
...
def main():
    ...
    print("M-bounded metric: M = number of candidate rows ODEPE returned per cell "
          "(its runtime-detected multiplicity) => mbounded == oracle over returned rows.")
    ...
    for est in ESTIMATORS:
        ...
        for cell_id, inst in cells.items():
            ...
            wtf, pmt, prim, rescue_path, source_type = load_metadata_notes(md_path)
            ...
            # M-bounded: argmin over the M candidate branches ODEPE returned. M is
            # the runtime-detected multiplicity, which ODEPE already used to truncate
            # result.csv -> M = number of returned rows. So this is oracle ranking over
            # the returned rows (no config, no overlay). For M=1 systems it equals top1.
            M = len(res_all) if res_all else 1
