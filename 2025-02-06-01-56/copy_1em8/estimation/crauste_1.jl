...
ic = [0.268, 0.249, 0.855, 0.692, 0.492]
p_true = [0.664, 0.125, 0.232, 0.597, 0.562, 0.29, 0.847, 0.591, 0.529, 0.572, 0.684, 0.35, 0.419]
...
data_sample = Dict(vcat("t", map(x -> x.rhs, measured_quantities)) .=> CSV.read("/home/orebas/julia/no-matlab-no-worry/2025-02-06-01-56/copy_1em8/data/crauste_1.csv", Tuple, header=false))
...
CSV.write("/home/orebas/julia/no-matlab-no-worry/2025-02-06-01-56/copy_1em8/estimation_results/crauste_1.csv", table, header=string.(collect(keys(table))))
