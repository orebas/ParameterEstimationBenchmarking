...
ic = [0.854, 0.607, 0.794, 0.852]
p_true = [0.29, 0.237, 0.459, 0.344, 0.771, 0.29, 0.502]
...
data_sample = Dict(vcat("t", map(x -> x.rhs, measured_quantities)) .=> CSV.read("/home/orebas/julia/no-matlab-no-worry/2025-02-06-01-56/copy_1em2/data/daisy-mamil4_4.csv", Tuple, header=false))
...
CSV.write("/home/orebas/julia/no-matlab-no-worry/2025-02-06-01-56/copy_1em2/estimation_results/daisy-mamil4_4.csv", table, header=string.(collect(keys(table))))
