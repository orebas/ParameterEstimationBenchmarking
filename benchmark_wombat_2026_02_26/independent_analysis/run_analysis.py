#!/usr/bin/env python3
"""
Independent Benchmark Analysis for benchmark_wombat_2026_02_26
Produces 8 CSV tables, 14 PNG figures, and a REPORT.md summary.

Usage:
    cd ~/ParameterEstimationBenchmarking/benchmark_wombat_2026_02_26/independent_analysis
...
INPUT_CSV = os.path.join(SCRIPT_DIR, "..", "analysis_results", "flat_results_with_metrics.csv")
...
# Domain classification for each ODE system
DOMAIN_MAP = {
    ...
    "slow_fast": "Biology",
    "two_compartment_pk": "Pharmacology",
...
def load_data():
    ...
    df["noise_cat"] = pd.Categorical(
        df["noise"].map(lambda x: f"{x:g}" if x > 0 else "0"),
...
def make_T02(df):
    ...
    for m in METHOD_ORDER:
        for noise in NOISE_ORDER:
            ...
            rows.append({
                "method": METHOD_LABELS[m],
                "noise": f"{noise:g}" if noise > 0 else "0",
                "n_rows": len(sub),
                "success_at_1pct": sub["success_at_1pct"].mean() * 100,
                "success_at_10pct": sub["success_at_10pct"].mean() * 100,
                "success_at_50pct": sub["success_at_50pct"].mean() * 100,
...
def make_T05(df):
    """T05: Polishing effect — nopolish vs polish per system x noise."""
    rows = []
    for sys_name in sorted(df["name"].unique()):
        for noise in NOISE_ORDER:
            np_sub = df[(df["name"] == sys_name) & (df["noise"] == noise) & (df["run"] == "odepe_nopolish")]
            p_sub = df[(df["name"] == sys_name) & (df["noise"] == noise) & (df["run"] == "odepe_polish")]
...
def make_T06(df):
    """T06: Replica stability per method."""
    rows = []
    for m in METHOD_ORDER:
...
def make_F02(df):
    pivot = df.groupby(["name", "run"])["success_at_10pct"].mean().unstack("run") * 100
    ...
    sns.heatmap(pivot, annot=True, fmt=".0f", cmap="RdYlGn", vmin=0, vmax=100,
                linewidths=0.5, ax=ax, cbar_kws={"label": "Success @ 10% (%)"})
    ax.set_title("Success Rate @ 10% by System and Method")
    ax.set_ylabel("")
    ax.set_xlabel("")
    fig_save(fig, "F02_method_comparison_heatmap.png")
...
def make_F04(df):
    ...
    p_rates = []
    for noise in NOISE_ORDER:
        np_sub = df[(df["run"] == "odepe_nopolish") & (df["noise"] == noise)]
        p_sub = df[(df["run"] == "odepe_polish") & (df["noise"] == noise)]
    ...
    ax.bar(x - width / 2, np_rates, width, label="ODEPE (no polish)",
           color=METHOD_COLORS["odepe_nopolish"], edgecolor="gray")
    ax.bar(x + width / 2, p_rates, width, label="ODEPE (polish)",
           color=METHOD_COLORS["odepe_polish"], edgecolor="gray")
...
def make_F08(df):
    ...
    fig.suptitle(f"Replica Spread at Noise = {noise_val} by System", fontsize=14, y=1.01)
    fig.tight_layout()
...
def generate_report(df, t01, t02, t03, t05):
    ...
    np_overall = df[df["run"] == "odepe_nopolish"]["success_at_10pct"].mean() * 100
    p_overall = df[df["run"] == "odepe_polish"]["success_at_10pct"].mean() * 100
...
- **AMIGO2** achieves the highest overall success rate at **{method_overall['amigo2_run']:.1f}%** (success@10%).
- **ODEPE (polish)** follows at **{method_overall['odepe_polish']:.1f}%**, with polishing providing a **+{polish_uplift:.1f} pp** uplift over the unpolished variant.
- **SciML** has the lowest success rate at **{method_overall['sciml_run']:.1f}%** but is the fastest method.
- Success degrades from **{succ_noise0:.1f}%** at noise=0 to **{succ_noise_high:.1f}%** at noise=0.01 (averaged across methods).
...
### Method Rankings
1. **AMIGO2** — Best overall accuracy ({method_overall['amigo2_run']:.1f}%), most noise-robust, but slowest (median {timing['AMIGO2']:.0f}s).
2. **ODEPE (polish)** — Second best ({method_overall['odepe_polish']:.1f}%), good accuracy-speed balance.
3. **ODEPE (no polish)** — Faster but less accurate ({method_overall['odepe_nopolish']:.1f}%).
4. **SciML** — Fastest method but lowest accuracy ({method_overall['sciml_run']:.1f}%).

### Key Takeaways
- **Noise is the primary difficulty driver** — success drops by ~{succ_noise0 - succ_noise_high:.0f} pp from noise=0 to noise=0.01.
