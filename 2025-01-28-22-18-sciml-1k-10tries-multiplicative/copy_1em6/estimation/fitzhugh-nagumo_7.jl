...
ic = [0.764, 0.875]
p_true = [0.838, 0.533, 0.839]
time_interval = [-1.0, 1.0]
datasize = 1001

sampling_times = range(time_interval[1], time_interval[2], length = datasize)

data_sample = Dict(vcat("t", map(x -> x.rhs, measured_quantities)) .=> CSV.read("/home/ademin/no-matlab-no-worry/2025-01-28-22-18/copy_1em6/data/fitzhugh-nagumo_7.csv", Tuple, header=false))
...
# optprob = Optimization.OptimizationProblem(optf, p_rand, lb = -3.0*ones(2+3), ub = 3.0*ones(2+3))
...
CSV.write("/home/ademin/no-matlab-no-worry/2025-01-28-22-18/copy_1em6/estimation_results/fitzhugh-nagumo_7.csv", table, header=string.(collect(keys(table))))
