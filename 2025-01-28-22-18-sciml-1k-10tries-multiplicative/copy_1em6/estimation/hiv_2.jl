...
ic = [0.439, 0.786, 0.194, 0.317, 0.423]
p_true = [0.498, 0.565, 0.293, 0.235, 0.788, 0.147, 0.476, 0.193, 0.466, 0.884]
time_interval = [-1.0, 1.0]
datasize = 1001

sampling_times = range(time_interval[1], time_interval[2], length = datasize)

data_sample = Dict(vcat("t", map(x -> x.rhs, measured_quantities)) .=> CSV.read("/home/ademin/no-matlab-no-worry/2025-01-28-22-18/copy_1em6/data/hiv_2.csv", Tuple, header=false))
...
# optprob = Optimization.OptimizationProblem(optf, p_rand, lb = -3.0*ones(5+10), ub = 3.0*ones(5+10))
...
CSV.write("/home/ademin/no-matlab-no-worry/2025-01-28-22-18/copy_1em6/estimation_results/hiv_2.csv", table, header=string.(collect(keys(table))))
