...
ic = [0.69, 0.104, 0.655, 0.836]
p_true = [0.399, 0.513, 0.811]
...
data_sample = Dict(vcat("t", map(x -> x.rhs, measured_quantities)) .=> CSV.read("/home/orebas/julia/no-matlab-no-worry/2025-02-06-01-56/copy_1em8/data/seir_4.csv", Tuple, header=false))
...
CSV.write("/home/orebas/julia/no-matlab-no-worry/2025-02-06-01-56/copy_1em8/estimation_results/seir_4.csv", table, header=string.(collect(keys(table))))
