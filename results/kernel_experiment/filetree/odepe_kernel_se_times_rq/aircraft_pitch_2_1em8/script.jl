...
name = "aircraft_pitch"
parameters = @parameters M_alpha M_q M_delta_e Z_alpha 
states = @variables  theta(t) q(t) alpha(t)
observables = @variables  y1(t)
state_equations = [
    D(theta) ~ q,
    D(q) ~ (-M_alpha*alpha - M_delta_e*sin(2.0*t) - 0.2*M_q*q) / 0.05,
    D(alpha) ~ (0.05*q - 0.002*Z_alpha*alpha) / 0.1,
]
measured_quantities = [
    y1 ~ 0.05*q,
]
ic = [0.723, 0.796, 0.883]
p_true = [0.157, 0.17, 0.116, 0.766]
