#!/usr/bin/env python3
"""
Build flat_results_with_metrics.csv for the numbat benchmark.
...
matching the bilby flat_results_with_metrics.csv schema (so downstream
run_analysis.py works), plus numbat-specific extra columns.

Usage:
    python3 results/numbat_analysis/build_flat_metrics.py [--bench-dir DIR]
"""
...
DEFAULT_BENCH = REPO_ROOT / "benchmark_numbat_2026-05-06"
...
def load_result_csv(p: Path):
    """Read per-cell result.csv (single row of estimated values keyed by var name)."""
    if not p.exists():
        return None
    try:
        df = pd.read_csv(p)
...
def main():
    ...
    ap.add_argument(
        "--out",
        default=str(REPO_ROOT / "results/numbat_analysis/flat_results_with_metrics.csv"),
    )
    args = ap.parse_args()
    ...
    for est in ESTIMATORS:
        ...
        for cell_id, inst in cells.items():
            ...
            res = load_result_csv(cell_path / "result.csv")
            ...
            med, mn, mx, rmse, s1, s10, s50 = compute_metrics(res, true_all, id_vars)
