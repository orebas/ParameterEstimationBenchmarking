...
ic = [0.479, 0.806, 0.687, 0.428]
p_true = [0.238, 0.186, 0.754]
...
data_sample = Dict(vcat("t", map(x -> x.rhs, measured_quantities)) .=> CSV.read("/home/orebas/julia/no-matlab-no-worry/2025-02-06-01-56/copy_1em8/data/seir_3.csv", Tuple, header=false))
...
CSV.write("/home/orebas/julia/no-matlab-no-worry/2025-02-06-01-56/copy_1em8/estimation_results/seir_3.csv", table, header=string.(collect(keys(table))))
