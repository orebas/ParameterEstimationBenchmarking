#!/usr/bin/env python3
"""
Independent Benchmark Analysis for benchmark_bilby_2026_03_09 (complete)
Produces 8 CSV tables, 14 PNG figures, and a REPORT.md summary.

All rankings and conclusions are data-driven (not hardcoded).

Usage:
    python3 results/bilby_analysis/independent_analysis/run_analysis.py
"""
...
def make_T06(df):
    ...
    for m in METHOD_ORDER:
        ...
        for (sys_name, noise), grp in sub.groupby(["name", "noise"]):
            ...
            if len(errs) >= 4:  # 8 replicas but some may be missing
