...
ic = [0.526, 0.361, 0.353, 0.458]
p_true = [0.494, 0.819, 0.248]
...
data_sample = Dict(vcat("t", map(x -> x.rhs, measured_quantities)) .=> CSV.read("/home/orebas/julia/no-matlab-no-worry/2025-02-06-01-56/copy_1em2/data/seir_7.csv", Tuple, header=false))
...
CSV.write("/home/orebas/julia/no-matlab-no-worry/2025-02-06-01-56/copy_1em2/estimation_results/seir_7.csv", table, header=string.(collect(keys(table))))
